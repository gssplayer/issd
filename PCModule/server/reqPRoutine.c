...
void* issd_compute_routine(void *arg)
{
	...
	fprintf(stdout, "infname = %s, outfname = %s, devname = %s, opType = %d, req_pid = %jd\n, reqSeq:%d\n",
			rPacket->ifname, rPacket->ofname, rPacket->devname, rPacket->opType, (intmax_t)rPacket->req_pid, rPacket->reqSeq );
	fflush(stdout);

	int ret = issd_compute(rPacket->opType, rPacket->ifname, rPacket->ofname, rPacket->devname);	

...
{
	int *pNum_packet = (int *)pCmd;
	*pNum_packet = (bufsize + SECTOR_SIZE - 1) / SECTOR_SIZE;
	
	fprintf(stdin, "num of wr packet  is %d\n", *pNum_packet);		
	...
	for(i =0; i < *pNum_packet; i++)
	{
		...
		memcpy(data, (void*)pCmd, SECTOR_SIZE);

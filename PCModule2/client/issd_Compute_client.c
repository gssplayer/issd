...
//	printf("ifname:%s, ofname:%s\n", ifname, ofname);
	...
	{
		snprintf(rPacket.ifname[0], MAX_FILENAME_LEN, "%s", rPathname);
		printf("ifname0 absolute pathname:%s:%s\n", ifname, rPacket.ifname[0]);
		free(rPathname);
	}
	...
	switch(opType)
	{
		...
		case ISSD_MERGE:
		{
			if((rPathname = realpath(ISSD_MERGE_va.va_ifname, NULL)) != NULL)
			{
				snprintf(rPacket.ifname[1], MAX_FILENAME_LEN, "%s", rPathname);
				printf("ifname1 absolute pathname:%s:%s\n", ISSD_MERGE_va.va_ifname, rPacket.ifname[1]);
				free(rPathname);
			}
			break;
		}

...
/*     while (g_break == 0); */
	...
	uart_printf("NUM_RW_BUFFERS: %d\n", NUM_RW_BUFFERS);
	uart_printf("NUM_RD_BUFFERS: %d\n", NUM_RD_BUFFERS);
	uart_printf("NUM_WR_BUFFERS: %d\n", NUM_WR_BUFFERS);
	led(0);
    sanity_check();
    //----------------------------------------
...
int issd_read_sectors(UINT32 buf_addr, UINT32 lba, UINT32 num_sectors)
{
    ...
    while (remain_sects != 0) {
        ...
        if (vpn != NULL) {
          nand_page_ptread(bank, vpn / PAGES_PER_BLK, 
									vpn % PAGES_PER_BLK, 
									sect_offset,
									num_sectors_to_read, buf_addr, /*RETURN_ON_ISSUE*/ RETURN_WHEN_DONE);
        }
        // The host is requesting to read a logical page that has never been written to.
        else {
		...
		buf_addr += (num_sectors_to_read * BYTES_PER_SECTOR);
        sect_offset   = 0;
        remain_sects -= num_sectors_to_read;
        lpn++;
    }

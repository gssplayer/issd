...
#define DRAM_BYTES_OTHER	((NUM_COPY_BUFFERS + NUM_FTL_BUFFERS + NUM_HIL_BUFFERS + NUM_TEMP_BUFFERS) * BYTES_PER_PAGE + BAD_BLK_BMP_BYTES \
							 ...
							 + ISSD_CMD_BYTES + ISSD_COMPUTING_BYTES)
...
#define ISSD_CMD_BUFFER_ADDR (BLK_ERASE_CNT_ADDR + BLK_ERASE_CNT_BYTES)
#define ISSD_CMD_BYTES BYTES_PER_PAGE

// dram<->flash for issd
#define ISSD_COMPUTING_ADDR (ISSD_CMD_BUFFER_ADDR + ISSD_CMD_BYTES)
...
// read/write for issd
int issd_read_sectors(UINT32 buf_addr, UINT32 lba, UINT32 num_sectors);

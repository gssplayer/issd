...
{
	uart_print("READ_BUFFER CMD execting.\n");
	
	mem_copy(HIL_BUF_ADDR, ISSD_CMD_BUFFER_ADDR, BYTES_PER_PAGE);
	
	afCommand_t * afCmd = NULL;
	afCmd = (afCommand_t *)HIL_BUF_ADDR;
	...
	//uart_printf("before return L opType = %d\n", read_dram_32(&(afCmd->opType)));
...
{
	//uart_print("In pio_sector_transfer.\n");
	disable_fiq();
	...
	//uart_print("exiting pio_sector_transfer.\n");
}
